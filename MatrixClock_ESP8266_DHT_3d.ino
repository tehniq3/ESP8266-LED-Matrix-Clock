...
char ssid[] = "namenetwork";                    // your network SSID (name)
...
void rtc_set(tm* tt) {
    ...
    digitalWrite(led, LOW);  // led off -> ok
}
//**************************************************************************************************
float rtc_temp() {
    float t = 0.0;
...
*/    
    if (cnt1h == 1) { //     // verify time (1 = every hour, 24 - every day)
        digitalWrite(led, HIGH);  // led is on (stay on until NTP server is read)
        f_tckr24h = true;
        cnt1h = 0;
    }
}
...
poz = poz+6;
            char2Arr(WT_arr[MEZ.WT][1], d_PosX - poz, 0);
poz = poz+6;           
            char2Arr(WT_arr[MEZ.WT][2], d_PosX - poz, 0);
poz = poz+6;
            char2Arr(WT_arr[MEZ.WT][3], d_PosX - poz, 0);
poz = poz+6;
...
poz = poz+6;
            ...
            char2Arr(M_arr[MEZ.mon12 - 1][1], d_PosX - poz, 0);
poz = poz+6;
            char2Arr(M_arr[MEZ.mon12 - 1][2], d_PosX - poz, 0);
poz = poz+6;
            char2Arr(M_arr[MEZ.mon12 - 1][3], d_PosX - poz, 0);
poz = poz+6;    
            char2Arr(' ', d_PosX - poz, 0);               
            char2Arr('.', d_PosX - poz, 0);
poz = poz+6;
